...
module Advent.Y2019.Day07 (part1, part2) where

import qualified Advent.Y2019.IntCode as IC
...
-- | A program that copies input to output (and never halts)
pipe :: IC.Computer
pipe = IC.fromList [3, 7, 4, 7, 1005, 1, 0, 0]

-- | Runs a list of IntCode Computers, piping the output of one into the input
-- of the next
pipeline :: [IC.Computer] -> [IC.Computer]
pipeline (a:b:cs) = a' : pipeline (b' : cs)
  where
    (a', output) = IC.readOutputAll $ IC.runProgram a
    b' = IC.addInputs b output
pipeline [a] = [IC.runProgram a]
pipeline [] = []

runAmplifiers :: ([IC.Computer] -> [IC.Computer]) -> IC.Computer -> [Int] -> Int
runAmplifiers pipelineFn c inputs =
  let cs = map (IC.addInput c) inputs  -- initialize all computers
      cs' = IC.addInput pipe 0 : cs    -- pipe 0 as the first input
  in snd . IC.readOutput1 . last . pipelineFn $ cs'

-- | >>> runAmplifiers1 (IC.fromString "3,15,3,16,1002,16,10,16,1,16,15,15,4,15,99,0,0") [4,3,2,1,0]
-- 43210
-- | >>> runAmplifiers1 (IC.fromString "3,23,3,24,1002,24,10,24,1002,23,-1,23,101,5,23,23,1,24,23,23,4,23,99,0,0") [0,1,2,3,4]
-- 54321
runAmplifiers1 :: IC.Computer -> [Int] -> Int
runAmplifiers1 = runAmplifiers pipeline

-- | >>> readFile "input/2019/day07.txt" >>= return . part1
-- 21860
part1 :: String -> Int
part1 input = foldl1' max $ map (runAmplifiers1 c) $ permutations [0..4]
  where c = IC.fromString input
...
-- | Runs a list of IntCode Computers, piping the output of one into the input
-- of the next, and piping the output of the _last_ back to the input of the
-- _first_.
pipelineCycle :: [IC.Computer] -> [IC.Computer]
pipelineCycle computers =
  if halted computers
     then computers
...
-- 139629729
runAmplifiers2 :: IC.Computer -> [Int] -> Int
runAmplifiers2 = runAmplifiers pipelineCycle

-- | >>> readFile "input/2019/day07.txt" >>= return . part2
-- 2645740
part2 :: String -> Int
part2 input = foldl1' max $ map (runAmplifiers2 c) $ permutations [5..9]
  where c = IC.fromString input
