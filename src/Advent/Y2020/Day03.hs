{- Toboggan Trajectory -}
module Advent.Y2020.Day03 (part1, part2) where
...
cellAt :: Row -> Int -> Cell
cellAt row pos =
  let pos' = pos `mod` V.length row
  in row V.! pos'

treesInPath :: Int -> (Int, Int) -> Field -> Int
treesInPath _ _ [] = 0
treesInPath pos (dx, dy) field@(row:_) =
  let n = case cellAt row pos of
            Tree -> 1
            Open -> 0
  in n + treesInPath (pos + dx) (dx, dy) (drop dy field)
