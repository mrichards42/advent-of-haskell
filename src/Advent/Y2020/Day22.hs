{- Crab Combat -}
module Advent.Y2020.Day22 (part1, part2) where
...
-- Tried this with Seq also, and lists were actually faster
type Card = Int
type Deck = [Card]
type DeckNE = NonEmpty Card
type Decks = (Deck, Deck)
type DecksNE = (DeckNE, DeckNE)

data Player = Player1 | Player2
data Winner = Winner { winnerDeck :: Deck
                     , winnerPlayer :: Player
                     }

pDeck :: Parser Deck
pDeck = do
  _ <- M.string "Player " <* M.anySingle <* M.string ":\n"
  L.decimal `P.sepBy1NG` M.newline

pDecks :: Parser Decks
pDecks = do
  p1 <- pDeck
  _ <- M.count 2 M.newline
  p2 <- pDeck
  return (p1, p2)

parseInput :: String -> Decks
parseInput = parseOrError pDecks
...
toDecksNE :: Decks -> DecksNE
toDecksNE (p1, p2) = (NE.fromList p1, NE.fromList p2)

playRound :: (DecksNE -> Player) -> DecksNE -> Decks
playRound roundFn decks@(a :| as, b :| bs) =
  case roundFn decks of
    Player1 -> (as ++ [a, b], bs)
    Player2 -> (as, bs ++ [b, a])
...
play1 :: DecksNE -> Player
play1 (a :| _, b :| _ ) = if a > b then Player1 else Player2

runGame1 :: Decks -> Winner
runGame1 (p1, []) = Winner p1 Player1
runGame1 ([], p2) = Winner p2 Player2
runGame1 decks = runGame1 . playRound play1 . toDecksNE $ decks

-- | >>> part1 <$> readFile "input/2020/day22.txt"
-- 33559
part1 :: String -> Int
part1 = deckScore . winnerDeck . runGame1 . parseInput
...
play2 :: DecksNE -> Player
play2 decks@(a :| as, b :| bs)
  -- recursive game
  | length as >= a && length bs >= b = winnerPlayer $ runGame2 (take a as, take b bs)
  -- normal game
  | otherwise = play1 decks

runGame2 :: Decks -> Winner
runGame2 initialDecks = go (initialDecks, Set.empty)
  where
    go :: (Decks, Set Decks) -> Winner
    go ((p1, []), _) = Winner p1 Player1
    go (([], p2), _) = Winner p2 Player2
    go (decks, prev)
      | decks `Set.member` prev = Winner (fst decks) Player1 -- short circuit
      | otherwise = go (playRound play2 (toDecksNE decks), Set.insert decks prev)

-- | >>> part2 <$> readFile "input/2020/day22.txt"
-- 32789
part2 :: String -> Int
part2 = deckScore . winnerDeck . runGame2 . parseInput
