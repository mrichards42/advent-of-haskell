{- Shuttle Search -}
module Advent.Y2020.Day13 (part1, part2) where
...
-- I've written this from scratch before, don't feel like doing it again
...
pTimestamp :: Parser Integer
pTimestamp = L.decimal

pBus :: Parser Bus
pBus = M.choice [ Just <$> L.decimal
                , Nothing <$ M.char 'x'
                ]

pBusses :: Parser [Bus]
pBusses = pBus `M.sepBy` M.char ','

parseInput :: String -> (Integer, [Bus])
parseInput = parseOrError parser
  where parser :: Parser (Integer, [Bus])
        parser = do
          t <- pTimestamp
          _ <- M.newline
          busses <- pBusses
...
waitTime now bus =
  let arrival = now - (now `mod` bus)
  in if arrival < now
        then arrival + bus - now
        else arrival - now

-- | >>> part1 "939\n7,13,x,x,59,x,31,19"
-- 295
-- >>> part1 <$> readFile "input/2020/day13.txt"
-- 1915
part1 :: String -> Integer
part1 input =
  let (now, allBusses) = parseInput input
      busses = catMaybes allBusses
      (wait, bus) = head . sortOn fst . map (\n -> (waitTime now n, n)) $ busses
  in wait * bus
...
-- | >>> part2 "939\n7,13,x,x,59,x,31,19"
-- 1068781
-- >>> part2 <$> readFile "input/2020/day13.txt"
-- 294354277694107
part2 :: String -> Integer
part2 input =
  let bussesWithPosition = busPositions . snd . parseInput $ input
      remainders = map busRemainder bussesWithPosition
      busses = map snd bussesWithPosition
  in fromJust $ chineseRemainder $ zip remainders busses

  where busPositions :: [Bus] -> [(Integer, Integer)]
        busPositions = catMaybes . zipWith (liftA2 (,)) (map Just [0..])

        -- Bus positions are _negative_ remainders, since we need to calculate
        -- times "ahead" (in the future, i.e. how many minutes are needed)
        -- rather than times "behind" (i.e. how many extra minutes), which is
        -- what positive remainders would get us.
        busRemainder :: (Integer, Integer) -> Integer
        busRemainder (pos, bus) = (- pos) `mod` bus
